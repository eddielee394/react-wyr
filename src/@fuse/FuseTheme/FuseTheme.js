...
const themesObj =
  Object.keys(themesConfig).length !== 0 ? themesConfig : defaults;
...
export let themes = Object.assign(
  ...
  ...Object.entries(themesObj).map(([key, value]) => {
    ...
    return {
      [key]: createMuiTheme(
        _.merge({}, muiTheme, { mixins: customMixins(muiTheme) })
      )
    };
  })
);
...
class FuseTheme extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    return !_.isEqual(prevState.theme, selected)
      ? { theme: themes[nextProps.selectedTheme] }
      : null;
  }
